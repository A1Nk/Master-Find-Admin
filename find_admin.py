...
try:
    print ColorsEnum.CYAN + r"""
           #########################################################################
           #        _______ _       ____       _       _           _ _   _____     #
           #       |__  ___(_)_ __ |  _ \     / \   __| |_ __ ___ (_) \ | ____|    #
           #         | |__ | | '_ \| | | |   / _ \ / _` | '_ ` _ \| |  \| |        #
           #         |  __|| | | | | |_| |  / ___ \ (_| | | | | | | | |\  |        #
           #         |_|   |_|_| |_|____/  /_/   \_\__,_|_| |_| |_|_|_| \_|        #
           #                                                                       #
           #"""+ ColorsEnum.RED + r"""                   ~TOOL UNTUK MENCARI PANEL ADMIN~ """+ ColorsEnum.CYAN + r"""                   #
           #"""+ ColorsEnum.WHITE + r"""                         Edited script By Aink  """+ ColorsEnum.CYAN + r"""                       #
           #                                                                       #
           #########################################################################
           """
    ...
    try:
        print ""
        site = raw_input(ColorsEnum.YELLOW + "Masukan Website?: """+ColorsEnum.WHITE + r"""""")
        site = site.replace("http://","")
        print ""
        print ("\tCheck website " + site + " ...")
        conn = httplib.HTTPConnection(site)
        conn.connect()
        print "\t[$] Oke... Server sedang Online."
    except (httplib.HTTPResponse, socket.error) as Exit:
        print ""
        raw_input("\t [!] Maaf terjadi kesalahan, Server offline atau URL tidak valid")
        exit()
    print ColorsEnum.BLUE + r""
    ...
    print "5 CGI"
    print "6 BRF"
    print ColorsEnum.RED + "\nTekan 1 dan 'Enter' untuk memilih PHP\n"
    print ""+ ColorsEnum.GREEN + r""
    code=input("> ")
    ...
    if code == 1:
        ...
        for admin in php:
            ...
            if response.status == 200:
                var1 = var1 + 1
                print ColorsEnum.YELLOW + "%s %s" % ( "\n\n>>>" + host, "Selamat, Admin Panel Ketemu!")
                raw_input(ColorsEnum.RED + "Tekan enter untuk melanjutkan scan \n")
            elif response.status == 404:
                var2 = var2
            elif response.status == 302:
                print ColorsEnum.RED + "%s %s" % ("\n>>>" + host, "Kemungkinan Panel Admin (302 - Redirect)")
            else:
                print ColorsEnum.RED + "%s %s %s" % (host, " Kemungkinan Merespon:? ", response.status)
            connection.close()
        print(ColorsEnum.CYAN + "\n\nCompleted \n")
        print var1, " Menemukan Panel Admin"
        print var2, " Total Website Scan"
